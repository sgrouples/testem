...
module.exports = View.extend({
  ...
  _showError(titleText, err) {
    ...
    this.setErrorPopupMessage(errMsgs);
  ...
  renderTop() {
    ...
    var mewe = this.config.get('mewe');

    var meweUrl = mewe.url.replace(/\/?$/, '/');
    meweUrl = (meweUrl.indexOf('://') === -1) ? 'http://' + meweUrl : meweUrl;

    var url = this.config.get('url');
    var cols = this.get('cols');

    var TEXT1 = '  dev server: ';
    var TEXT2 = 'tests server: ';

    screen
      .position(0, 1)
      .write(pad((new StyledString(mewe.title).foreground('blue')) + ' (' + mewe.endpoint + ')', cols, ' ', 1))
      
      .position(0, 2)
      .write(TEXT1)
      .position(TEXT1.length + 1, 2)

      .display('underscore')
      .write(meweUrl, cols - TEXT1.length, ' ', 1)
      .display('reset')

      .position(meweUrl.length + TEXT1.length + 1, 2)
      .write(pad('', cols - TEXT1.length - url.length, ' ', 1))
      
      .position(0, 3)
      .write(TEXT2)

      .display('underscore')
      .write(url, cols - TEXT2.length, ' ', 1)
      .display('reset')

      .position(url.length + TEXT2.length + 1, 3)
      .write(pad('', cols - TEXT2.length - url.length, ' ', 1));
